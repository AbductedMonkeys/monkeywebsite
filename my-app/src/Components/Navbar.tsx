...
function Navbar() {
  ...
  <div className={`shadow-md w-full ${homeURL !== '/' ? "bg-lGray" : "bg-grimace"} fixed top-0 left-0`}>
      ...
      <ul className={`md:flex md:items-center md:pb-0 pb-12 absolute md:static ${homeURL !== '/' ? "bg-lGray" : "bg-grimace"} md:z-auto z-[-1] left-0 w-full md:w-auto md:pl-0 pl-9 ease-in ${open ? 'top-16 ':'top-[-490px]'}`}>
        {
          Links.map((link)=>(
            <li key={link.name} onClick={() =>setOpen(!open)} className='md:ml-8 text-3xl md:my-0 my-7'>
              <HashLink to={`${link.link}`} className='text-mPink hover:text-mTan cursor-pointee'>{link.name}</HashLink>
            </li>
          ))
        }
      </ul>
