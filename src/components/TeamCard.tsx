...
const renderTeamLinks = (links) => {
  ...
  const renderGameIcons = (games) => {
    return Object.keys(games).map((game, index) => {
      switch (game) {
        case 'tornado':
          return games[game] ? <Image src={tornadoImg} alt={'tornado icon'} key={`${game}-${index}`} className=' flex h-6 md:h-5 w-auto m-2' /> : null;
        case 'lockjaw':
            return games[game] ? <Image src={lockjawImg} alt={'lockjaw icon'} key={`${game}-${index}`} className='flex h-6 md:h-5 w-auto m-2' /> : null;
        default:
          return null;
      }
    });
 ...
 const TeamCard = ({person}) => {
    return (
        <> 
            ...
            <div className="bg-grimace m-6 p-2 flex flex-wrap justify-left h-full rounded-md outline-4 outline md:outline-8 outline-mTan">
            ...
            <div className="flex flex-col p-4 w-full lg:w-2/3">
                <div className="text-left justify-center flex flex-col h-4/6">
                    <h1 className="font-bold text-xl flex flex-shrink w-full">{person?.name}</h1>
                    <h2 className="font-bold text-sm flex flex-shrink w-full">{person?.title}</h2>
                </div>
